...
%	\end{enumerate}
	...
	\end{enumerate}
	If $E=\Omega$, $X(\omega)=\omega$, then $Q_{\cG}$ is called a {\bf regular conditional probability}\index{regular conditional probability}. 
	...
	\begin{proposition}
		\begin{enumerate}[(i)]
			...
			\begin{equation}
				\bE (XY|\cG)=\bE (X|\cG)Y. 
			\end{equation}
			\item (Jesen's inequality) If $\varphi$ is a {\bf convex}\index{convex} function, then 
			\begin{equation}
 ...
 If $X_n\in \cF_n$  is a submartingale then it can be uniquely decomposed as $X_n=M_n+A_n$, where $M_n\in \cF_n$ is martingale, $A_0=0$, $A_{n+1}\geq A_n$ almost surely and $A_n$ is $\cF_{n-1}$-measurable.
	...
	The celebrated {\bf Doob-Meyer decomposition}\index{Doob-Meyer decomposition} says that, under mild hypotheses, a submartingale can be decomposed into a martingale plus an increasing process. We are limiting ourselves to continuous submartingales, and the theorem we want to state is the following.
...
%    \end{remark}
    ...
    \begin{exercise}
    	Let $h\in W^{1,2}_0$. Porve that 
    	\[
    	    \frac{\bP\l( \|\omega-h\|\leq \eps\r)}{\bP\l(\|\omega\|\leq \eps \r)}\to \exp \l(-\frac{1}{2}\int_0^1 |\dot h_s|^2 \d s \r). 
    	\]
    \end{exercise}
